...
- EDIT: `code/modules/shuttle/shuttle.dm` : Исправлен баг исчезновения пола в ангарах при взлете сабшатла
  - Добавлена fallback логика для случаев, когда `underlying_turf_area` пустой
  - Определение правильного типа пола на основе области (hangar/space/plasteel)
  - Решает проблему непостоянного поведения при взлете сабшатлов с основных кораблей

- EDIT: `code/modules/shuttle/on_move.dm` : [CELADON-EDIT] Безопасное сохранение пола ангара при взлёте сабшатла
  - Перед `ScrapeAway` оценка целевого слоя: если в ангаре получался бы `space`, но ниже есть не-space слой, уменьшаем глубину срезания на 1
  - Не чинит реально разрушенный пол (если ниже только space — изменений нет)
  - Минимальный и безопасный фикс для снижения вероятности «дыр» без изменения общей логики
