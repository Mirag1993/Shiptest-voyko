		...
		if(C.get_item_by_slot(ITEM_SLOT_HEAD) == src)
			if(HAS_TRAIT_FROM(src, TRAIT_NODROP, REVERSE_BEAR_TRAP_TRAIT) && !struggling)
				...
				switch(time_left)
					...
					if(50 to 60)
						fear_string = ""
				C.visible_message(span_danger("[C] fiddles with and pulls at [src]...</span>", \
				span_danger("You [fear_string] try to pull at [src]...</span>", "<i>You hear clicking and ticking.</i>")
				...
				if(!prob(escape_chance))
					...
					user.visible_message(span_warning("The lock on [user]'s [name] pops open!</span>", \
					span_userdanger("You force open the padlock!</span>", "<i>You hear a single, pronounced click!</i>")
...
/obj/item/reverse_bear_trap/attack(mob/living/target, mob/living/user)
	if(target.get_item_by_slot(ITEM_SLOT_HEAD))
		...
		return
	target.visible_message(span_warning("[user] starts forcing [src] onto [target]'s head!</span>", \
	span_userdanger("[target] starts forcing [src] onto your head!</span>", "<i>You hear clanking.</i>")
	...
	if(!do_after(user, 30, target = target) || target.get_item_by_slot(ITEM_SLOT_HEAD))
		return
	target.visible_message(span_warning("[user] forces and locks [src] onto [target]'s head!</span>", \
	span_userdanger("[target] locks [src] onto your head!</span>", "<i>You hear a click, and then a timer ticking down.</i>")
...
/obj/item/reverse_bear_trap/proc/snap()
	...
	if(!istype(H) || H.get_item_by_slot(ITEM_SLOT_HEAD) != src)
		...
		jill.visible_message(span_boldwarning("[src] goes off in [jill]'s mouth, ripping [jill.p_their()] head apart!</span>", span_userdanger("[src] goes off!"))
		jill.emote("scream")
