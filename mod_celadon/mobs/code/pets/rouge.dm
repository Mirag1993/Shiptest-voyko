...
// 		return
// 	if(!item_to_add)
// 		user.visible_message(span_notice("[user] pets [src].</span>", span_notice("You rest your hand on [src]'s head for a moment."))
...
// 		else if(user)
// 			user.visible_message(span_notice("[user] надевает [item_to_add] на центральную голову [real_name]. [src] смотрит на [user] и довольно шипит.</span>",
// 				span_notice("Вы надеваете [item_to_add] на голову [real_name]. [src] озадачено смотрит на вас, пока другие головы смотрят на центральную с завистью.</span>",
