...
/obj/structure/bed/roller/future/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/roller/future/robo))
		...
		if(has_buckled_mobs())
			...
			R.loaded = src
			forceMove(R)
			user.visible_message(span_notice("[user] collects [src].</span>", span_notice("You collect [src]."))
		return 1
	else
		return ..()
...
/obj/structure/bed/roller/future/MouseDrop(over_object, src_location, over_location)
	...
	if(over_object == usr && Adjacent(usr))
		...
		if(has_buckled_mobs())
			return 0
		usr.visible_message(span_notice("[usr] collapses \the [src.name].</span>", span_notice("You collapse \the [src.name]."))
...
/obj/item/roller/future/attackby(obj/item/I, mob/living/user, params)
	if(istype(I, /obj/item/roller/future/robo))
		...
		if(R.loaded)
			...
			return
		user.visible_message(span_notice("[user] loads [src].</span>", span_notice("You load [src] into [R]."))
...
/obj/item/roller/future/robo/deploy_roller(mob/user, atom/location)
	if(loaded)
		...
		user.visible_message(span_notice("[user] deploys [loaded].</span>", span_notice("You deploy [loaded]."))
		loaded = null
	else
