...
/obj/item/examine(mob/user)
	...
	if(check_donator(user))
		. += span_notice("<ul>This is one of your donator items, to <b>[(donator_allow_other_usage ? "allow" : "disallow")]</b> sharing <b>CtrlShiftClick</b> it.</ul></span>"
	else
		if(!donator_allow_other_usage)
			. += span_warning("A strange force prevents you from making eye contact with it.</span>"
